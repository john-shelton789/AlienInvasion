...
class HighScores:
    def __init__(self, screen):
        ...
        self.high_scores_list = open("images/high_scores.txt", "r")
        self.pointsfont = pygame.font.SysFont(None, 64)
        self.scorefont = pygame.font.SysFont(None, 48)
        self.titlesurface = self.pointsfont.render('HIGH SCORES', False, (78, 244, 66))
    ...
    # Show the scores
    def show_score(self):
        self.screen.blit(self.titlesurface, (self.screen_rect.centerx - 150, 150))
        loopindex = 0
        # display the high scores in a list
        for score in self.scores_list:
            if score == 0:
                break
            score_y = 250 + (loopindex * 40)
            scoresurface = self.scorefont.render(str(score), False, (255, 255, 255))
            self.screen.blit(scoresurface, (self.screen_rect.centerx - 50, score_y))
            loopindex += 1
